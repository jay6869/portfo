...
import { projects, writeups, type Project } from "@/lib/data";
...
export const Route = createFileRoute("/projects/$slug")({
  ...
  head: ({ loaderData, params }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.project.title} — Janith Godage` },
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-4 py-24 text-center">
      <div className="mono text-sm text-muted-foreground">project not found.</div>
      <Link to="/projects" className="mono mt-4 inline-block text-[color:var(--signal)]">← back to projects</Link>
    </div>
  ),
...
function ProjectDetail() {
  ...
  const related = writeups.filter((w) =>
    w.tags.some((t) => project.tags.some((pt) => pt.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(pt.toLowerCase()))),
  ).slice(0, 3);

  return (
    <article className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
      <Link to="/projects" className="mono inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-[color:var(--signal)]">
      ...
      {related.length > 0 && (
        <Section title="related writeups">
          <div className="grid gap-3 sm:grid-cols-2">
            {related.map((w) => (
              <Link key={w.slug} to="/writeups/$slug" params={{ slug: w.slug }}
